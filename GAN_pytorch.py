...
def show_result(G_net, z_, num_epoch, show=False, save=False, path='result.png'):
    ...
    """
    epochs_to_save = {5, 10, 20, 50, 100}
    if num_epoch in epochs_to_save:
        data_fake = G_net(z_)
        # Example of how to print a single image. Use plt.subplot to combine all images in a single plt.
        # If save = True, then save the image using  plt.savefig(path)
        fig = plt.figure(figsize=(8, 8))
        for i in range(1, 26):
            fig.add_subplot(5, 5, i)
            plt.imshow(np.reshape(data_fake[i-1].detach().numpy(), (28,28)), cmap='gray')
        fig.savefig(path, dpi=fig.dpi, bbox_inches='tight', pad_inches=0.5)
        if show:
            plt.show()
...
if __name__ == '__main__':
    ...
    train_loader = DataLoader(dataset=train_data, batch_size=batch_size, shuffle=True, num_workers=20)
    ...
    plt.figure()
    plt.plot(train_hist['G_losses'], '-b', label='generator loss')
    plt.plot(train_hist['D_losses'],'-r', label='discriminator loss')
    plt.legend()
    plt.savefig('G-D loss.png')
    plt.show()
